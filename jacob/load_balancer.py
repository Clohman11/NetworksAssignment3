...
def list2str(lis):
    ...
    string = ""
    for i in range(len(lis)):
        string = string + lis[i] + " " #Use spaces as the delimiter for each address
    string = string + "\n" #Bit of a hack, strings need a newline when going into a pipe
    return string;

def str2list(string):
    lis = list(string.split(" "))
    del lis[-1] #Also a bit of a hack, removes last null character in list
    return lis;
        ...
        for i in range(len(SERVERS)):
            ...
            for j in range(len(result)): #Repeat for delay (in milliseconds)
                ...
                if x:
                    ...
                    delay.insert(i, x)
                    break
            try: #If delay wasn't set, it's because regex couldn't find the value, meaning all packets were dropped
                if delay[i]:
                    pass
            except:
                delay.insert(i, 99999) #Simulate infinite RTT with reasonably large value
...
def client_helper(preference, connection, sender_address):
