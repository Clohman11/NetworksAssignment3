...
try:
    ...
    print("Cannot reach webpage. Please include the full url, inlcuding protocol, and check spelling e.g. https://www.google.com")
    sys.exit()
...
def server():
    while(True):
        ...
        data = connection.recv(32) #Header length always fixed, struct allows dynamic size payload within size limit (pads extra bytes)
        Sender, Type, Message = unpack(data)
        if Sender == 10:
            pid = os.fork()
            if pid == 0:
